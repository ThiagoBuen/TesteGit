...
Core::Core(string _LN, string _ULN, Process *_PP, unsigned int _Index, GW *_PGW, HT *_PHT, string _Path):Block(_LN,_PP,_Index,_Path)
{
	PGW=_PGW;
	PHT=_PHT;
	...
	tsmiup1->Initialization("tsmiup1","MEAN_ARITHMETIC","BDelay since instantiation at arriving on PGCS up to the final -run --evaluate for message type 1 (seconds)",1);
			...
			if (Run != 0)
				{
					...
					if (Keywords.size() == KeywordHashes.size() && Keywords.size() > 1)
						{
							...
							// Publish binding < Hash("IoTTestApp"), App PID >
					...
					// Publish binding < Hash("IoTTestApp"), "IoTTestApp" >
					NewTernaCommandLineArgument(1,HashProcessLegibleName,"IoTTestApp",PCL);
					...
					// Publish binding < PID, HashPLN >. Added in August 2018 to solve the lack of peer discovery problem
					NewTernaCommandLineArgument(3,PP->GetSelfCertifyingName(),HashProcessLegibleName,PCL);
				}
			else
				{
					S << Offset <<  "(ERROR: Unable to obtain the scheduled message at index zero)" << endl;
...
{
	// ***************************************************************
	// Set the keywords that will be used to find out this app
	// ***************************************************************

	Keywords.push_back("IoTTestApp"); 			// The process legible name must the first name on the vector
	...
	Keywords.push_back("Application");
