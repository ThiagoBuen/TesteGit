...
int CoreRunExpose01::Run(Message *_ReceivedMessage, CommandLine *_PCL, vector<Message*> &ScheduledMessages, Message *&InlineResponseMessage)
{
	int 			Status=OK;
	string			Offset = "                    ";
	unsigned int 	NA=0;
									...
									if (Limiter.at(0) == PB->PP->GetDomainSelfCertifyingName())
										{
											...
											PB->S << Offset <<  "(Number of PSS(s) is "<<PCore->PSSTuples.size()<<")"<<endl;
													...
													if (Terna.size() > 0)
														{
															...
															// Generate the ng -p --b
															PMB->NewCommonCommandLine("-p","--b","0.1",PB->StringToInt(Terna.at(0)),Terna.at(1),Values,Publish,PCL);
											...
											// Added in April 2014. Expose the relation with the words Host and OS to enable search for tuples at process object

											Values->clear();

											Values->push_back(PB->PP->GetHostSelfCertifyingName());

											PMB->NewCommonCommandLine("-p","--b","0.1",9,"Host",Values,Publish,PCL);

											Values->clear();

											Values->push_back(PB->PP->GetOperatingSystemSelfCertifyingName());

											PMB->NewCommonCommandLine("-p","--b","0.1",2,"OS",Values,Publish,PCL);
											...
											PB->S << Offset <<  "(There is a message generated for time = "<<GetTime()<<")" << endl;
