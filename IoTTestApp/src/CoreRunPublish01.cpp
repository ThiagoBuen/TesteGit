...
int CoreRunPublish01::Run(Message *_ReceivedMessage, CommandLine *_PCL, vector<Message*> &ScheduledMessages, Message *&InlineResponseMessage)
{
	int 			Status=OK;
	string			Offset = "                    ";
	Core			*PCore=0;
							...
							if (PeerData.size() == 3)
								{
									...
									PB->PP->NewMessage(GetTime(),1,true,"Not_Necessary",PeerData.at(2),"Not_Necessary",PB->GetPath(),Publish);
